...
- Experience: pinned main-scroll horizontal journey (400vh outer, sticky h-screen viewport; vertical scroll drives track x, then page continues). Travel spans exact first-card-centered → last-card-centered offsets measured via ResizeObserver + load + fonts.ready (a mount-only measure under-read the range when CSS parsed late — verified in headless Chromium: x 282 → -782, dead-center focus at both ends, no page overflow). Dwell zones remap travel to progress [0.1, 0.9] (clamped) so the first card rests centered after pin-engage and the last rests centered before release, symmetric in reverse — verified: x holds +282 across f=0→0.1 and -782 across f=0.9→1. Nodes jump via page-scroll math; continuous center-distance fade kept; static stack under reduced motion; tall expanded cards capped at 82vh with local scroll fallback. Perf: card fade runs on MotionValues (`JourneyCard`, zero per-frame setState — node index is the only state and updates discretely), and pinned cards use `.glass-flat` instead of blurred glass since backdrop-blur repaints every frame while translating.
- Projects: full-bleed stacking deck (custom section shell WITHOUT SectionWrapper's min-height — that was injecting ~550px of dead space between heading and deck; measured gap 402px → 282px, remainder is the inherent card centering in the pinned viewport; deck as full-width sibling; cards opaque `bg-secondary` with `animateOnView={false}` inside deck so only the parent transform moves them; `overflow-x-clip` wrapper so the sticky pin survives). Reworked to a diagonal one-card journey: each card owns its center-moment (i/(n-1)), distance from it drives bottom-right → center → top-left with matched fade/scale, so cards never overlap (max 2 visible, always mirrored opposites); travel remapped to [0.1,0.9] so first/last rest centered before release (mirrored reverse). Verified in headless Chromium across 5 travel points + 21-step overlap audit; static list under reduced motion. Lightbox renders via createPortal to document.body so no ancestor transform/stacking quirk can ever shrink the fixed overlay or misplace the panel.
...
- Type system: `font-display` → Playfair Display (titles, quotes, headings), `font-body` → Source Sans Pro (body/nav/buttons/labels); Tektur appears in exactly two places via `font-tektur` — the navbar brand mark and the hero name (legacy `font-aldrich` utility retained in config but unused). Palette simplified to blue + pink: all `neon-green` usages converted (hero name/ring/chevron, buttons, chips, eyebrows, nodes, dots, dates, overlays, `--motto-ink`/`--glow-green` → blue equivalents); achievement ranking → pink and incubation → blue so categories stay distinct. `--neon-green` token + `glass-glow-green` + glow-prop plumbing remain defined but unused (removal would risk silent Tailwind misses). Follow-up purge: all pink usages converted to blue (buttons, chips, eyebrows, spine/nodes/dots, dates, overlays, form errors, close buttons, motto eyebrow/divider; achievement ranking → blue like placement/incubation); drop-shadows removed from the motto eyebrow + quote. `--neon-pink` token + `glass-glow-pink` remain defined but unused. Page-wide computed-style scan confirms zero pink/green pixels.
- Portfolio: first real item — CuanCraft UX Case Study tile (bundled cover `src/assets/images/porfolio cover/…-Cover.png`, 1920×1080) opens a two-column case modal: name + `year · team` (values only, no field labels) + localized 3-paragraph description (`portfolio.items.uiux.description` via `ta()`) + PDF button on the left, full cover (`object-contain`, never cropped) on the right; panel is max-w-5xl with its own scroll on small screens, close button kept for the focus trap. Second real item: Jagad Agro Wonoboyo (`JAGO-COVER.png`, 1440×1080) with localized 2-paragraph description (`portfolio.items.jagad.*`) and its Drive link behind a "View portfolio file" button (`portfolio.viewPortfolio`; per-file label via `entry.pdfLabel`). Gallery is now multi-category: entries carry `id` + `categories[]` (Jagad spans websites/dashboards/uiux/product, captioned by its primary), filters match on inclusion. Third real item: Point Property Solo (`Point-property Cover.png`, 1440×1080, UI/UX only) with localized 2-paragraph description (`portfolio.items.point.*`) and its Drive link behind the "View portfolio file" button. The case modal is solid (no glass) in both themes via `.lightbox-case`: dark surface + light text in dark mode, white + black text in light mode (media well tinted per theme). The PDF button opens the Google Drive link (bundled 23MB PDF removed from dist).
